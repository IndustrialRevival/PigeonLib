...
public final class InventoryUtils {
    ...
    public static boolean fits(@Nonnull Inventory inv, @Nonnull ItemStack item, int... slots) {
        ...
        for (int slot : slots) {
            ...
            if (isValidStackSize(stack, item, inv) && ItemUtils.isItemSimilar(stack, item)) {
                return true;
            }
        }
        ...
        for (ItemStack item : items) {
            ...
            for (int slot : slots) {
                ...
                if (stack == null || stack.getType() == Material.AIR) {
                    cache.put(slot, item);
                    resolved = true;
                } else if (isValidStackSize(stack, item, inv) && ItemUtils.isItemSimilar(stack, item)) {
    ...
    /**
     * This method removes a given number of items from a given {@link Inventory}
