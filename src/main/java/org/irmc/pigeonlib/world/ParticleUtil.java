...
import org.irmc.pigeonlib.java.CollectionUtil;
    ...
    public static void drawLineByTotalAmount(@NotNull Particle particle, int totalAmount, @NotNull Location... locations) {
        for (int i = 0; i < locations.length; i++) {
            if ((i + 1) < locations.length) {
                ...
                double[] x = CollectionUtil.disperse(totalAmount, location1.getX(), location2.getX());
                double[] y = CollectionUtil.disperse(totalAmount, location1.getY(), location2.getY());
                double[] z = CollectionUtil.disperse(totalAmount, location1.getZ(), location2.getZ());
